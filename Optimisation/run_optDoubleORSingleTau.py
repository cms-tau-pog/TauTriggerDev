...
def run_optimization(dataset_rate, dataset_eff, rate_budget):
    ...
    def f(a):
        ...
        N_den, N_num = dataset_rate.get_Nnum_Nden_DoubleORSinglePNet(a)
        rate = (N_num/N_den)*L1A_physics

        N_den, N_num = dataset_eff.ComputeEffAlgo_DoubleORSinglePNet(a)
    ...
    res = minimize(f, [0.67, 0.57], bounds=((0.5, 0.8), (0.4, 0.7)), method="L-BFGS-B", options={"eps": [0.01, 0.01]})
    return res.x
...
if __name__ == '__main__':
    ...
    optim_param = run_optimization(dataset_rate, dataset_eff, rate_budget = 47)
    optim_param = np.append(optim_param, [PNetparam_t3])

    print('Optimisation finished:')
    print("Optimized parameters:", optim_param)

    N_den, N_num = dataset_rate.get_Nnum_Nden_DoubleORSinglePNet(optim_param)
    rate_opt = (N_num/N_den)*L1A_physics

    print("Approx. Rate:", rate_opt)

    N_den, N_num = dataset_eff.ComputeEffAlgo_DoubleORSinglePNet(optim_param)
